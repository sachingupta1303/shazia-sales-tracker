...
export function renderDailyReportHtml(report: DailyReport, dateLabel: string): string {
  ...
  const rowsHtml = rows.map((r, i) => {
    ...
    return `
      <tr style="background:${i % 2 ? "#f8fafc" : "#ffffff"};">
        <td style="padding:7px 8px;color:#9ca3af;font-size:11px;text-align:center;">${i + 1}</td>
        <td style="padding:7px 8px;font-weight:600;color:#0f172a;">${escapeHtml(r.buyerName)}</td>
        <td style="padding:7px 8px;color:#475569;">${escapeHtml(r.country)}</td>
        <td style="padding:7px 8px;text-align:center;"><span style="background:${tc.bg};color:${tc.fg};font-size:10px;font-weight:700;padding:2px 6px;border-radius:5px;">${tierLabel}</span></td>
        <td style="padding:7px 8px;text-align:right;color:#475569;">${n1(r.monthTarget)}</td>
        <td style="padding:7px 8px;text-align:right;font-weight:600;color:#0f172a;">${n0(r.monthActual)}</td>
        <td style="padding:7px 8px;text-align:right;color:#475569;">${n1(r.weekTarget)}</td>
        <td style="padding:7px 8px;text-align:right;font-weight:600;color:#0f172a;">${n0(r.weekActual)}</td>
        <td style="padding:7px 8px;text-align:right;font-weight:700;color:${sc.fg};">${pct}</td>
        <td style="padding:7px 8px;text-align:center;"><span style="background:${sc.bg};color:${sc.fg};font-size:10px;font-weight:700;padding:2px 7px;border-radius:10px;white-space:nowrap;">${STATUS_LABEL[r.status]}</span></td>
        <td style="padding:7px 8px;text-align:right;font-weight:700;color:#0f172a;">${n0(r.yearTarget)}</td>
      </tr>`
  }).join("")
  ...
  <div style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;background:#f1f5f9;padding:18px;">
    <div style="max-width:980px;margin:0 auto;background:#ffffff;border-radius:14px;overflow:hidden;border:1px solid #e2e8f0;">
      ...
      <div style="padding:6px 22px 4px;color:#64748b;font-size:11px;">
        Sort: <b>Critical first</b> → key accounts (T1→T2→T3→Others) → poorest %.
        Till-now % is pace-adjusted (Till-week actual ÷ Till-week target).
      </div>

      <div style="padding:8px 16px 20px;overflow-x:auto;">
